...
import pro.anuj.challenge.pc.api.config.AppConfig;
import pro.anuj.challenge.pc.api.elasticsearch.ElasticSearchService;
import pro.anuj.challenge.pc.api.response.ErrorMessage;
...
abstract class StreamHandler implements RequestHandler<Map<String, Object>, Response> {
    ...
    private final RestHighLevelClient client = new RestHighLevelClient(RestClient.builder(httpHost).setHttpClientConfigCallback(
            httpClientBuilder -> {
                if (config.isAwsElasticSearch()) {
                    httpClientBuilder.addInterceptorLast(RequestSigningInterceptor.PROD_INTERCEPTOR);
                }
    ...
    private Response<ErrorMessage> handleError(String msg, int status) {
        return new Response<>(new ErrorMessage(config.toString(), status), APPLICATION_JSON, status);
    }
